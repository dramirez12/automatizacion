...
if ($visualizacion == 0) {
  ...
  if (isset($_POST['cb_jornada'])) {
    ...
    if ($jornada == "0") {
      ...
      $sql_tabla_control = json_decode(file_get_contents("http://desarrollo.informaticaunah.com/automatizacion/api/asistencia_charla_api.php?control=" . $jornada), true);
    }
  } else {
    $_SESSION['impartida'] = "";
    $counter = 0;

      $sql_tabla_control = json_decode(file_get_contents("http://desarrollo.informaticaunah.com/automatizacion/api/asistencia_charla_api.php"), true);
  }
}
