...
header("location: ../contenidos/editarRoles-view.php?msj=1&Rol=$Rol2 ");*/
    ...
    $sql = "call proc_actualizar_actividades('$actividad','$descripcion','$horas','$comision','$_SESSION[usuario]','$id_actividad' )";
    ...
    $valor = "select actividad, descripcion, horas_semanales, id_comisiones  from tbl_actividades WHERE id_actividad= '$id_actividad'";
    $result_valor = $mysqli->query($valor);
    $valor_viejo = $result_valor->fetch_array(MYSQLI_ASSOC);

    if ($valor_viejo['actividad'] <> $actividad and $valor_viejo['descripcion'] <> $descripcion  and $valor_viejo['horas_semanales'] <> $horas and $valor_viejo['id_comisiones'] <> $comision) {

        $Id_objeto = 74;
        bitacora::evento_bitacora($Id_objeto, $_SESSION['id_usuario'], ' MODIFICO', ' LA ACTIVIDAD ' . $valor_viejo['actividad'] . ' POR ' . $actividad . ', LA DESCRIPCION DE LA ACTIVIDAD ' . $actividad . ' ');
