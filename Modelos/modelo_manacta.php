...
if ($_POST['estado-acta'] == 'nuevo') {
    try {
        $stmt = $mysqli->prepare("INSERT INTO tbl_estado_acta (estado) VALUES (?)");
        ...
        $accionac = 'INSERTO';
        $descripcionac= 'nuevo estado para el acta con nombre:'.$estado;
        $fechaac = $hoy;
...
if ($_POST['estado-acta'] == 'actualizar') {
    try {
        $stmt = $mysqli->prepare('UPDATE tbl_estado_acta SET estado = ? WHERE id_estado = ?');
        ...
        $id_userac = $_SESSION['id_usuario'];
        $accionac = 'MODIFICO';
        $descripcionac= 'estado para el acta con nombre:'.$estado;
        $fechaac = $hoy;
        $stmt = $mysqli->prepare("INSERT INTO `tbl_bitacora` (`Id_usuario`, `Id_objeto`, `Fecha`, `Accion`, `Descripcion`) VALUES (?,?,?,?,?)");
        $stmt->bind_param("iisss", $id_userac, $id_objetoac, $fechaac, $accionac, $descripcionac);
        $stmt->execute();
...
if ($_POST['estado-acta'] == 'eliminar') {
    ...
    try {
        $stmt = $mysqli->prepare('DELETE FROM tbl_estado_acta WHERE id_estado = ? ');
        ...
        $id_userac = $_SESSION['id_usuario'];
        $accionac = 'ELIMINO';
        $descripcionac= 'estado para el acta con id:'.$id_borrar;
        $fechaac = $hoy;
        $stmt = $mysqli->prepare("INSERT INTO `tbl_bitacora` (`Id_usuario`, `Id_objeto`, `Fecha`, `Accion`, `Descripcion`) VALUES (?,?,?,?,?)");
        $stmt->bind_param("iisss", $id_userac, $id_objetoac, $fechaac, $accionac, $descripcionac);
        $stmt->execute();
